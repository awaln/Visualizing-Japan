...
var gallerySort = (function(){
	...
	function nameToUrl(name){
		...
		return("<div class='vjxContainer'><img src='https://studio.edx.org/c4x/VJx/VJx/asset/" + name + "'></div>");
	}
	...
	$(document).ready(function(){
		var vjxCounter = 0;
		for(i in imageNames){
			$('#vjxMain').append("<div class='vjxContainer' id='" + vjxCounter + "'><img src='https://studio.edx.org/c4x/VJx/VJx/asset/" + imageNames[i] + "'></div>");
			state['array'].push(vjxCounter);
			state['array'].push('vjxContainer');
			vjxCounter += 1;
		}
		...
		$('.vjxContainer').hover(function(){
			//put on the overlay
			$(this).prepend("<div id='overlay'><div id='zoomblock'><img src='https://studio.edx.org/c4x/VJx/VJx/asset/vjxZoomIcon.png'/></div><div id='modtag'>Modern Women</div><div id='tradtag'>Traditional and Cosmetics</div><div id='hometag'>Home</div><div id='cinetag'>Cinema and Montage</div></div>");
			...
			$('#zoomblock img').hover(function(){
				...
				$('#zoomblock').on('click',function(){
					...
					$('#vjxMain').prepend("<div id='lightbox'>Click to Close<img src='" + toZoom + "'/></div>");
    ...
    function updateMaterials() { //this runs to return to previous state.
    	$(".vjxContainer").each(function(){
    		$(this).removeClass().addClass('vjxContainer'); //viciously remove all the classes
    		var id = $(this).attr('id');
    		$(this).addClass(state['array'][id*2+1]);
    	});
    }

    function getState() {
    	console.log('returning getState');
