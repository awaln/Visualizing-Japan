...
$(document).ready(function(){
	...
	$('.vjxContainer > img').each(function(){ //constrain by largest dimension
		if($(this).height() > $(this).width()){
			$(this).css({'height':'200px','width':'auto'});
		}
		else{
			$(this).css({'height':'auto','width':'200px'});
		}
	});
	...
	$('.vjxContainer').hover(function(){
		...
		$('#blushtag, #eyetag, #curvetag, #camtag, #rectag').click(function(){
			...
			if($(this).parents('.vjxContainer').children(checkClass).length){ //how to test if element exists: Does it have a length?
				$(this).parents('.vjxContainer').children(checkClass).animate({'width':'130px'},100,function(){$(this).parents('.vjxContainer').children().remove(checkClass);});
			}
			else{
				...
				$(this).parents('.vjxContainer').children(checkClass).animate({'width':'160px'},100);
			}
